...
log(`  Line Manager:    suresh@demo.com / demo123`);
log('');
log('API Keys (for org-level task push):');
log(`  Org broadcast:   demo-org-key-99999`);
log(`  Unit push:       demo-push-key-12345`);
log('');
log('Features:');
log('  • Kanban board with drag-and-drop (4 columns)');
log('  • 3 task layers: Org (API) / Unit (Sr. Manager) / Team (self)');
log('  • Admin panel for org structure management');
log('  • Team Dashboard with Heatmap');
log('  • 1-on-1 notes with auto-generated action items');
log('  • Today\'s Focus smart nudges');
log('  • Notification bell with live polling');
log('  • Recurring tasks engine');
log('');
log(`Documentation: ${CYAN}docs/PRD.md, docs/HLD.md, docs/LLD.md, docs/USERGUIDE.md${RESET}`);
log('');
